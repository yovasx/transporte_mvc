...
// Define green icon for destination marker
var greenIcon = new L.Icon({
    iconUrl: '<?php echo BASE_URL; ?>/public/assets/images/mapa/parada-de-autobus-green.png',
    shadowUrl: '<?php echo BASE_URL; ?>/public/assets/plugins/leaflet/images/marker-shadow.png',
    iconSize:     [25, 41], 
    iconAnchor:   [12, 41], 
    popupAnchor:  [1, -34], 
    shadowSize:   [41, 41]
...
<?php foreach($paradas as $parada): ?>
    <?php if($parada['estado'] == 'Activo' && $parada['latitud'] && $parada['longitud']): ?>
        ...
        // store parada id and original popup content
        marker.paradaId = <?php echo $parada['id_parada']; ?>;
        marker.originalPopupContent = '<b><?php echo addslashes($parada['nombre_parada']); ?></b><br><small>Lat: <?php echo $parada['latitud']; ?>, Lng: <?php echo $parada['longitud']; ?></small>';
        markers.push(marker);
    <?php endif; ?>
<?php endforeach; ?>

// Event listener to update marker icons and popups on destination selection
$('#destino').on('change', function() {
    var selectedId = parseInt($(this).val(), 10);

    // Reset all markers to original icon and popup content
    markers.forEach(function(marker) {
        marker.setIcon(L.icon({
            iconUrl: '<?php echo BASE_URL; ?>/public/assets/images/mapa/parada-de-autobus.png',
            shadowUrl: '<?php echo BASE_URL; ?>/public/assets/plugins/leaflet/images/marker-shadow.png',
            iconSize:     [25, 41],
            iconAnchor:   [12, 41],
            popupAnchor:  [1, -34],
        ...
        marker.setPopupContent(marker.originalPopupContent);
    });

    // If a valid destination is selected, update its marker
    if (!isNaN(selectedId)) {
        var destinoMarker = markers.find(m => m.paradaId === selectedId);
        if (destinoMarker) {
            destinoMarker.setIcon(greenIcon);
            destinoMarker.setPopupContent('Destino');
            destinoMarker.openPopup();

            // Restore original icon and popup when popup is closed
            destinoMarker.on('popupclose', function() {
                destinoMarker.setIcon(L.icon({
                    iconUrl: '<?php echo BASE_URL; ?>/public/assets/images/mapa/parada-de-autobus.png',
                    shadowUrl: '<?php echo BASE_URL; ?>/public/assets/plugins/leaflet/images/marker-shadow.png',
                    iconSize:     [25, 41],
                    iconAnchor:   [12, 41],
                ...
                destinoMarker.setPopupContent(destinoMarker.originalPopupContent);
            });
        }
    }
});
